...
def _pull_llm_text(
    prompt_config: dict[str, Any], fallback: dict[str, Any], fields: tuple
) -> dict[str, Any]:
...
class BaseCrew(ABC):
    ...
    def _load_agents(self, inputs: dict[str, Any]) -> tuple:
        ...
        for name, spec in agent_specs.items():
            ...
            langfuse_name = _namespaced(
                _AGENT_PROMPT_NAMESPACE, prompt_key, source=f"agents/{name}.yaml"
            )
            ...
            for skill_name in spec.get("skills") or []:
                ...
                if not (skill_path / "SKILL.md").is_file():
                    raise ValueError(f"skill directory {skill_path} is missing SKILL.md")
    ...
    def _load_tasks(self, agents: dict[str, Agent], inputs: dict[str, Any]) -> tuple:
        ...
        for filename in self._task_yaml_names:
            ...
            langfuse_name = _namespaced(
                _TASK_PROMPT_NAMESPACE, prompt_key, source=f"tasks/{filename}"
            )
            ...
            tasks.append(
                Task(
                    ...
                    expected_output=expected_output.format(**safe_inputs)
                    if expected_output
                    else "",
    ...
    def _build_prompt_meta(
        ...
        for name, p in agent_prompts.items():
            ...
            prompt_meta[f"agent.{name}.prompt_source"] = (
                "langfuse" if p.version != "fallback" else "yaml_fallback"
            )
        ...
        for name, p in task_prompts.items():
            ...
            prompt_meta[f"task.{name}.prompt_source"] = (
                "langfuse" if p.version != "fallback" else "yaml_fallback"
            )
